...
function addLeaderStatsMessages(messages, leaders) {
	...
	['skills', 'bosses', 'activities'].forEach(category => {
		...
		if (item) {
			...
			messages.push(
				`Highest ${capitalizeWords(item.metric)} is ${item.player.displayName} | ${label}: ${value} | Rank: ${rank}.`,
			);
		}
	});
}
...
function selectRandomProperty(obj) {
	const validEntries = Object.values(obj).filter(
		value => value.level > 0 || value.kills > 0 || value.score > 0,
	);
